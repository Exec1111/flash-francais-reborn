 ...
 * - Affiche un Chip coloré avec le statut d'extraction PDF d'une ressource
 ...
 */
const PdfExtractionStatusChip = ({ resourceId, fileType, filePath, size = 'small', autoFetch = false }) => {
  ...
  const fetchStatus = async () => {
    ...
    setError(null);
    try {
      const data = await resourceService.getPdfExtractionStatus(resourceId);
      setStatus(data?.status || null);
    } catch (err) {
      setError(err?.response?.data?.detail || err?.message || "Erreur statut d'extraction PDF");
    } finally {
      setLoading(false);
    }
  };
...
export default PdfExtractionStatusChip;
